...
cred = credentials.Certificate("/home/dronepi/dronecred.json")
...
for key, value in data.items():
    print(data[key])
    if ((data[key]['dronestatus']['dronestatus']) == "none") and (data[key]['route']['route'] == 'Not Delivered'):
        route = 'pres'
        ...
        shop_long = 81.519054
        break
    elif (((data[key]['route']['route'] == 'shop') and (data[key]['dronestatus']['dronestatus']) == "Not Delivered")):
        route = 'shop'
        ph_num = (data[key]['userDetails']['phone'])
        username = (data[key]['userDetails']['username'])
...
if route == 'pres':
    ...
    db.reference("/" + username + "/" + 'dronearrived/dronearrived').set("true")

    time.sleep(30)

    ref = db.reference(f"/{username}/otpverified/otpverified")
    value = ref.get()

    print(value)

    # db.reference("/" + username + "/" + 'otpverified/otpverified')
...
db.reference("/" + username + "/" + 'dronestatus/dronestatus').set("Delivered")
